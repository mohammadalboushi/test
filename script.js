...
document.getElementById('file-avatar-input').addEventListener('change', async e => {
  ...
  try {
    ...
    const res = await fetch('https://pixeldrain.com/api/file', { method: 'POST', body: formData });
    const data = await res.json();
    if (data.success) {
      const textUrl = 'https://pixeldrain.com/api/file/' + data.id;
      await db.ref('users/' + myProfile.uid).update({ photo: textUrl });
      myProfile.photo = textUrl; localStorage.setItem('myProfile', JSON.stringify(myProfile));
...
function buildMsgEl(msg, isBackground = false) {
  ...
  if (msg.type === 'text') {
    ...
    // تم التعديل لدعم Catbox حيث لا يوجد خادم مصغرات: نضع المصغرة كفيديو صامت أو خلفية افتراضية
    let thumbUrl = msg.url;
    if(msg.url.includes('cloudinary')) {
       thumbUrl = msg.url.replace('/upload/', '/upload/w_400,h_300,c_fill,so_0/').replace(/\.[^/.]+$/, ".jpg");
    } else {
       // بالنسبة لـ Catbox سنستخدم خلفية سوداء مع أيقونة التشغيل لأن Catbox لا يولد صور مصغرة
       thumbUrl = ''; 
    }
    const fileSize = msg.size ? `<div class="video-meta-badge">${msg.size} MB</div>` : '';
    bubble.innerHTML = `${replyHtml}<div class="video-thumb-container" style="background:#000;">
        ${thumbUrl ? `<img src="${thumbUrl}" class="msg-img" onerror="this.style.display='none';" style="pointer-events: auto;"/>` : `<video src="${msg.url}#t=0.1" class="msg-img" style="pointer-events: auto; object-fit: cover;" preload="metadata"></video>`}
          ...
          xhr.onload = async function() {
            try {
              ...
              if ((xhr.status === 200 || xhr.status === 201) && data.success) {
                  ...
                  await pushMessage({ type: 'voice', url: textUrl, duration: finalDuration, senderUid: currentUser.uid, timestamp: Date.now(), replyTo: replyData });
              } else {
                handleFail('فشل الرفع', 'السيرفر رفض المقطع');
              }
            } catch (err) {
                handleFail('خطأ بالسيرفر', err.message);
            }
          };

          xhr.onerror = function(e) { handleFail('خطأ اتصال بالشبكة', 'Network Error / CORS'); };
...
function uploadMediaWithUI(file, type, extraData = null) {
  ...
  const tryUpload = () => {
    ...
    const handleFail = (msg, devDetails = '') => {
        console.error("Pixeldrain Media Upload Error:", msg, devDetails);
        if (row) { 
          const ov = row.querySelector('.pending-overlay'); 
    ...
    xhr.onload = async () => {
      try {
        ...
        if ((xhr.status === 200 || xhr.status === 201) && data.success) {
          const textUrl = 'https://pixeldrain.com/api/file/' + data.id;
          window.lastUploadedUrl = textUrl;
          if (type === 'image' && window.localImageCache) window.localImageCache[textUrl] = tempUrl;
          const currentRow = document.getElementById('row_' + tempId);
          if (currentRow) currentRow.remove();
          await pushMessage({ type: type, url: textUrl, duration: extraData?.duration || null, senderUid: currentUser.uid, timestamp: Date.now(), replyTo: tempMsg.replyTo });
        } else {
          handleFail('فشل الرفع', 'السيرفر رفض الملف');
        }
      } catch (err) {
        handleFail('خطأ في الاستجابة', err.message);
      }
    };
    
    xhr.onerror = (e) => {
        console.error("Pixeldrain Network Error Details:", e);
        handleFail('خطأ اتصال', 'Network Error');
    };
    xhr.ontimeout = () => handleFail('انتهى الوقت', 'Timeout 180s');
...
async function toggleRecording(isSinging = false) {
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      // 🚀 تم رفع المهلة لأن سيرفرات Catbox قد تستغرق وقتاً أطول قليلاً
      xhr.timeout = 120000;
      ...
      xhr.onload = async function() {
        ...
        const textUrl = xhr.responseText.trim();
        if (xhr.status === 200 && textUrl.startsWith('http')) {
            let replyData = null;
            if (replyingToMsg) {
              replyData = { key: replyingToMsg.key, text: replyingToMsg.type === 'text' ? replyingToMsg.text : replyingToMsg.type === 'image' ? '📷 صورة' : '🎙️ صوت' };
              cancelReply();
...
function uploadLargeMediaWithXHR(file, fileSizeMB, mediaType, extraDuration = null) {
  ...
  const tryUpload = () => {
      ...
      const handleFail = (msg, devDetails = '') => {
          console.error("Pixeldrain Large Media Error:", msg, devDetails);
          if (row) { 
            const ov = row.querySelector('.pending-overlay'); 
            if (ov) ov.innerHTML = `<button onclick="window.pendingUploads['${tempId}']()" style="background:var(--bg-surface); border:1px solid var(--neon-pink); color:var(--neon-pink); padding:8px 12px; border-radius:12px; cursor:pointer; font-family:var(--font-ar); font-size:12px; font-weight:bold; box-shadow:var(--shadow-pink); max-width:90%; white-space:normal; line-height:1.5;">${msg}<br><span style="font-size:10px; color:#fff; opacity:0.8; display:block; margin-top:4px;" dir="ltr">${devDetails}</span> 🔄</button>`; 
          }
      ...
      xhr.onload = async function() {
        try {
          ...
          if ((xhr.status === 200 || xhr.status === 201) && data.success) {
            ...
            handleFail('استجابة غير صالحة', 'السيرفر رفض الملف');
          }
        } catch(err) {
            handleFail('فشل تحليل الرد', err.message);
        }
      };

      xhr.onerror = function() { handleFail('مشكلة شبكة', 'Network Error'); };
      xhr.ontimeout = function() { handleFail('انتهى وقت الانتظار', 'Timeout'); };
